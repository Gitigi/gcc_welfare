...
export default class PaymentDistribution extends Component {
	...
	getColor(year,month) {
		...
		if(!p)
			return '';
		if(!this.payments[p.date]) {
			this.payments[p.date] = {'color': colors[this.paymentCount % 5],date: Date(p.date)};
			this.paymentCount++;
		}
		
		return this.payments[p.date]
	}
						...
						{this.state.rows.map((year,index)=>{
							return <tr key={year}>
									...
									{months.map((m,index)=>{
										...
										return <td key={year+''+index} bgcolor={details.color} data-original-title={details.date}
